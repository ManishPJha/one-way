...
const getAllEmployees = async (req: Request, res: Response, next: NextFunction) => {
    ...
    new ApiFeature(EmployeeModel.find(), req.query)
    .sort(-1)
    .search("name")
    .filter()
    .pagination(limit)
    ...
    if (!(Employee.length > 0)) return next(new ErrorHandler("No records found!", 404))
...
const getSingleEmployee = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!Employee) return next(new ErrorHandler("No record found with this id!", 404))
...
const createEmployee = catchAsyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
...
const updateEmployee = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!isEmployeeExist) return next(new ErrorHandler("No record found with this id!", 404))
...
const deleteEmployee = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        res.status(200).json({
            success: true,
            message: `Employee id ${req.params.id} has been removed successfully.`
        })
